...
func (s *Storage) ArchiveEntries(status string, days int) (int64, error) {
	...
	query := `
		...
		SET
			status='removed'
		WHERE
			id=ANY(SELECT id FROM entries WHERE status=$1 AND starred is false AND share_code='' AND created_at < now () - '%d days'::interval ORDER BY created_at ASC LIMIT 50000000)
	`
